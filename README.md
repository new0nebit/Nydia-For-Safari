<h1 align="center">
  <picture>
    <source srcset=".docs/images/nydia-leaf-dark.svg" media="(prefers-color-scheme: light)" height="44px" alt="Nydia Logo">
    <img src=".docs/images/nydia-leaf-light.svg" height="44px" alt="Nydia Logo">
  </picture>

<div align="center">

[![Safari](https://img.shields.io/badge/Safari-WebExtension-555555?logo=safari&logoColor=white&labelColor=006CFF)](https://apps.apple.com/app/)
[![WebAuthn](https://img.shields.io/badge/Standard-WebAuthn%20%7C%20FIDO2-00B4D8?logo=webauthn&logoColor=white)](https://www.w3.org/TR/webauthn-3/)
[![Passkeys](https://img.shields.io/badge/Authentication-Passkeys-2EA44F?logo=data:image/svg+xml;base64,)](https://www.w3.org/TR/webauthn-3/) <br>
[![Sia](https://img.shields.io/badge/Storage-Sia%20Network-00C65E?logo=data:image/svg+xml;base64,)](https://sia.tech/)
[![License](https://img.shields.io/badge/License-MIT-green)](https://github.com/new0nebit/Nydia-For-Safari/blob/main/LICENSE)

</div>
</h1>

<div align="center">
    <img src=".docs/images/menu.png" height="630px" alt="Nydia Logo">
</div>
<br>
<p align="center">
  Nydia is a fast and lightweight Safari extension for decentralized passkey storage on the <a href="https://sia.tech">Sia network</a>.
  <br>
  Users can hold their passkeys locally with completely private decentralized backup.
</p>
<br>

## Preparing for building

#### System Requirements
- Safari 14+
- Xcode 16+

#### Build Prerequisites
- Node.js
- npm
- Xcode Command Line Tools

## How to build Nydia for Safari

1. Clone the repository:
```
git clone https://github.com/new0nebit/Nydia-For-Safari.git
```

2. Install dependencies:
```
npm install
```

3. Build the extension:
```
npm run build
```

4. Build the Xcode project:
```
xcodebuild -project "Nydia/Nydia Passkey Holder.xcodeproj" -scheme "Nydia Passkey Holder" -configuration Release -quiet
```

5. Load the extension in Safari browser:

    - Open Settings
    - Navigate to `Developer`
    - Enable "Allow unsigned extensions"
    - Go to `Extensions` and turn on Nydia: Passkey Holder

> [!NOTE]
> Versions for Chrome and Firefox are available <a href="https://github.com/new0nebit/Nydia-Passkey-Holder">here</a>.

## Configuration

#### Sia Renterd Setup

1. Install and configure `renterd` following the [official documentation](https://docs.sia.tech/renting/setting-up-renterd).
2. In the extension, configure your renterd settings:
    - Server Address
    - Server Port
    - Password
    - Bucket Name

<div align="center">
    <img src=".docs/images/renterd-settings.png" height="380px" alt="Nydia Logo">
</div>

## Security Notes

- Passkeys are stored locally using browser encryption.
- No passkey data is transmitted to third-party servers.
- The extension requires minimal permissions.

<br>

> [!IMPORTANT]
> The Nydia Passkey Holder is in an early stage of active development.<br>
> Passkey registration and authentication through Nydia may not be supported on all websites yet.<br>
> Handle with care.